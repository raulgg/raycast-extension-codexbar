...
- Never hand-edit provider entries in `src/providers/registry.ts` or files in `assets/provider-icons/`. They are synced from upstream by `npm run upstream:check` and `npm run upstream:sync-icons`. Transcribe new catalog values from the upstream Swift descriptors and name the upstream file and SHA you verified against in your pull request.
