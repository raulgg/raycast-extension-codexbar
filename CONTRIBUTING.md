...
- Never hand-edit provider entries in `src/providers/registry.ts` or redraw files in `assets/provider-icons/`. They are synced from upstream by `npm run upstream:check` and `npm run upstream:sync-icons`. When upstream removes a provider, delete its catalog entry, its SVG, and any other leftover that names it. The icon script reports a stale SVG and leaves the file in place. Transcribe new catalog values from the upstream Swift descriptors and name the upstream file and SHA you verified against in your pull request.
